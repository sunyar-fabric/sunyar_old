...
const CreatePlanDialog = ({ showDialog, closeDialog, getPlans, planId, planInfo }) => {
    ...
    const handleCreatePlan = async () => {
        ...
        const body = { planName, description, planNature, neededLogin, parentPlanId: planId, icon: '0', fDate, tDate }

        setLoadingDialog(true)
        try {
            let { status } = await HttpService.post('/api/sunyar/plan/plan', body)
            if (status === 200) {
        ...
        >
            <DialogContent style={{
                ...
                <div className="container d-flex flex-column justify-content-center align-items-center">
                    ...
                    <p className="border-bottom w-100 text-right pb-3"><span className="m-3"> ایجاد طرح</span></p>

                    <input type="text" id="planName" name="planName" className="bg-input-dialog mt-3 w-100" placeholder="عنوان اصلی" />

                    <div className="mt-3 w-100">
                        <DatePicker
                            value={planInfo?.fDate ? timestampToPersian(new Date(planInfo?.fDate).getTime()) : null}
                            minDate={planInfo?.fDate ? timestampToPersian(new Date(planInfo?.fDate).getTime()) : null }
                            locale={persian_fa}
                            editable={false}
                            calendar={persian}
                            inputMode='none'
                    ...
                    <div className="mt-3 w-100">
                        <DatePicker
                            value={planInfo?.tDate ? timestampToPersian(new Date(planInfo?.tDate).getTime()) : null}
                            maxDate={planInfo?.tDate ? timestampToPersian(new Date(planInfo?.tDate).getTime()) : null }
                            locale={persian_fa}
                            editable={false}
                            calendar={persian}
                            inputMode='none'
                    ...
                    <div className="d-flex mt-5"><a className="btn btn-info ml-3 px-5" onClick={handleCreatePlan}>ذخیره</a><a className="btn btn-outline-info text-info px-5" onClick={closeDialog}>انصراف</a></div>
                </div>

...
const {
  ...
  getPlanByParentPlan,
  loadPlanTreeAll,
} = require("../atomicServices/plan");
const {
  loadNeedyToPlan,
} = require("../../../plan/needyToPlan/atomicServices/needyToPlan");
const {
  loadSuccorCash,
} = require("../../../plan/succorCash/atomicServices/succorCash");

const wsCreatePlan = async (context) => {
  let loadUniques = await loadPlan(
  ...
  context = await createPlan(
    setContextInput(context, {
      ...
      isFinal: context.params.isFinal,
      parentPlanHashCode,
    })
  );
  context.result = {
...
const wsLoadPlanPaginate = async (context) => {
  context = await loadPlanPaginate(
    setContextInput(context, {
      planId: context.params.planId,
      planName: context.params.planName,
      planNature: context.params.planNature,
...
const wsLoadPlanTree = async (context) => {
  if (!context.params.all || context.params.all == "false") {
    context = await loadPlanTree(setContextInput(context));
  } else {
    context = await loadPlanTreeAll(setContextInput(context, {}));
  }
...
const wsDeletePlan = async (context) => {
  ...
  context = await getPlanByParentPlan(
    setContextInput(context, { planId: context.params.planId })
  );
  ...
  context = await deletePlan(
    setContextInput(context, { planId: context.params.planId, planHashCode })
  );
  if (context.output === 0) {
    throw createError(GlobalExceptions.plan.PlanNotFound);
  }

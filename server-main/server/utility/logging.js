...
const logging = async (req, res, next) => {
    ...
    context.request = {
        body: req.body, url: req.url, headers: req.headers
    };
    req.context = context;
    return context;
}
...
const saveLog = async (req, res, context, next) => {
    res.on("finish", () => {
        context.response = {
            body: res.body, headers: res.getHeaders(), statusCode: res.statusCode, statusMessage: res.statusMessage
        };
    })
    const data = {
    ...
    fs.appendFileSync(`../log/${fileName}`, JSON.stringify(data, null, 2) + "\n")
    next();
}


module.exports = { setContextInput, setContextOutput, createContext, saveLog, logging };
